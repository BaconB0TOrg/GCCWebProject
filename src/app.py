...
def post_create_server():
  ...
  if form.validate_on_submit():
    ...
    docker_id = mcdocker.make_server(name=form.name.data, port=port, max_players=str(
        form.maxPlayers.data), gamemode=str(form.gamemode.data.lower()))
    ...
    server = Server(name=form.name.data, description=str(form.description.data), docker_id=str(
        docker_id), owner_id=user.id, max_players=int(form.maxPlayers.data), port=port, tags=tags)
