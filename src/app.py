...
def get_log_out():
  if request.method != 'GET':
    print(
        f"[WARN] User tried to {request.method} to {url_for('get_log_out')}. Only GET is allowed")
...
def get_account():
  ...
  if not user:
    print(
        f"[WARN] Failed to GET {url_for('get_account')}: user {user_id} doesn't exist")
    flash('You have to be logged in to see that page.')
    return redirect(url_for('get_login'))

  form = ChangeEmailForm()
  print(
      f"[INFO] Successfully accessed {url_for('get_account')} for user {user_id}")
...
def post_create_server():
  if not session.get('logged-in'):
    print(
        f'[INFO] Anonymous user tried to {request.method} {url_for("post_create_server")}')
...
def get_update_server(server_id):
  if not session.get('logged-in'):
    print(
        f'[INFO] Anonymous user tried to {request.method} {url_for("get_update_server")}')
    flash("You must be logged in to do that!")
    return redirect(url_for('get_login'))
  ...
  if not server or server.owner_id != user_id:
    if not server:
      ...
      print(
          f'[WARN] User {user_id} tried updating Server {server_id}, but does not own it.')
    flash("You don't have permission to do that!")
    return redirect(url_for('list_server'))
...
def post_update_server(server_id):
  if not session.get('logged-in'):
    print(
        f'[INFO] Anonymous user tried to {request.method} {url_for("post_update_server")}')
    flash("You must be logged in to do that!")
    return redirect(url_for('get_login'))
  ...
  if form.validate_on_submit():
    ...
    if not server_as_list or len(server_as_list) == 0:
      print(
          f'[WARN] Failed to update server: User {user_id} does not own Server {server_id}!')
      flash("You have to own the server in order to update it!")
      return redirect(url_for('get_create_server'))
...
def delete_server(server_id):
  ...
  if session.get('user-id') == server.owner_id:
    ...
    print(
        f'[WARN] User {session.get("user-id")} tried to delete server {server_id}')
    flash("You don't have permission to do that!")
    return redirect(url_for('list_server'))
...
def get_terminal(server_id):
  ...
  if server.owner_id != int(session.get('user-id')):
    print(
        f'User {session.get("user-id")} failed to access server {server_id}: Not the owner')
    flash("You don't have permission to do that!")
    return redirect(url_for('welcome'))
